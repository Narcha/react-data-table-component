...
import { DataTableState, Action, Row } from './types';

export function tableReducer(state: DataTableState, action: Action): DataTableState {
	switch (action.type) {
		case 'UPDATE_ROWS': {
			const { rows } = action;
		...
		case 'SELECT_ALL_ROWS': {
			...
			if (mergeSelections) {
				const selections = allChecked
					? [...state.selectedRows, ...rows.filter((row: Row) => !isRowSelected(row, state.selectedRows, keyField))]
					: state.selectedRows.filter((row: Row) => !isRowSelected(row, rows, keyField));
		...
		case 'SELECT_MULTIPLE_ROWS': {
			...
			if (mergeSelections) {
				const selections = [
					...state.selectedRows,
					...selectedRows.filter((row: Row) => !isRowSelected(row, state.selectedRows, keyField)),
				];
