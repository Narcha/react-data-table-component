...
function DataTable(props: DataTableProps): JSX.Element {
	...
	const defaultSortColumn = React.useMemo(() => getColumnById(defaultSortFieldId, columnsMemo), [
		columnsMemo,
		defaultSortFieldId,
	]);
	...
	const [
		...
		dispatch,
	] = React.useReducer<React.Reducer<DataTableState, Action>>(tableReducer, initialState);
	...
	const handleSelectAllRows = (action: AllRowsAction) => {
		dispatch(action);
	};

	const handleSelectedRow = (action: SingleRowAction) => {
		dispatch(action);
	};
					...
					<Table disabled={disabled} className="rdt_Table" role="table">
						{showTableHead() && (
							<TableHead className="rdt_TableHead" role="rowgroup">
								...
								>
									...
									{columnsMemo.map((column: DataTableColumn) => (
										<TableCol
											key={column.id}
											column={column}
											rows={rows}
						...
						{!progressPending && rows.length > 0 && (
							<TableBody
								...
								role="rowgroup"
							>
								{calculatedRows.map((row: Row, i: number) => {
